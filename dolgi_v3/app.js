function App ( e, D ) {
    ...
    function A ( e, a ) {
        Array.from( e.attributes ).forEach( i => {
            ...
            if ( n === ':input' ) {
                const s = C( `( ${ /[.\[\]]/.test( v ) ? v : `data[ '${ v }' ]` } = ___VALUE___ );`, a, '___VALUE___,el, ev' )
            ...
            if ( n[ 0 ] === '@' ) {
                ...
                if ( E = /([\w_$]+)?(\((.+)\))?(!)?/.exec( v ) ) {
                    ...
                    f = ev => { EV_CH ( I ); FN[ I ].apply( e, A && A( D, e, ev, IT && ev.changedTouches ? ev.changedTouches[ 0 ]: ev ) ) !== null && CH() }
                }
                
                e.addEventListener( N, f, e[ 4 ] ? true : false )
            } else
    ...
    function EV_CH ( n ) {
        if ( !FN[ n ] ) {
            throw Error ( `Event '${ n }' not exists` );
        }
    }
